...
*************************************
* Name:	StandardisedRates_v3_timesplit
* Creator:	RMJ
* Date:	2021-04-15
* Desc:	Estimate age-sex standardised mortality/incidence rates. Uses ir, stptime,
*		and user-written command distrate. Separately for those with more or less than 2y follow-up.
* Version History:
*	Date	Reference	Update
*	20210415	StandardisedRates_v3	Tidy file to clarify what is happening
*	20210415	StandardisedRates_v3	Add loop to separate by time period
*	20210415	StandardisedRates_v3	Make agecats wider
*************************************
...
keep patid cohort index ageindex sex endreason6 enddate6 cod_L1 

** Create variables
egen agecat = cut(ageindex), at(18, 45(10)75, 100) icodes label // first group wide as few events
gen start = index
...
gen duration = (enddate6 - index) / 365.25
drop if duration <= 0 // keep if positive duration
...
** Use stsplit to split dataset into two time periods
stset time, fail(died) scale(365.25) id(patid)
stsplit split, at(2)
recode split (0=1)
*** New variables: split, _d, _t, _t0


** Open loop for period
forval P=1/2 {

	** Open loop for outcomes
	forval OUT=1/2 {

		** Copy to new frame
		frame change default
		capture frame drop live
		frame put if split==`P', into(live) 
		...
		** Use different frames to handle the two sets of results
		...
		*** Export as table
		...
		export delim using "outputs/standardised_v3_`name'_time`P'.csv", replace
