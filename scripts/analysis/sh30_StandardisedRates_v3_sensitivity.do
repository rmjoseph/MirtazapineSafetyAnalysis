* Created 2021-05-06 by Rebecca Joseph at the University of Nottingham
*************************************
* Name:	sh30_StandardisedRates_v3_sensitivity
* Creator:	RMJ
* Date:	20210506
* Desc:	Runs the standardised rates file on sensitivity analyses (using enddate 30 and self-harm only)
* Version History:
*	Date	Reference	Update
*	20210312	StandardisedRates_v2_sensitivity	Created file
*	20210325	sh30_StandardisedRates_v2_sensitivity	Exclude people with stop==start
*	20210506	sh30_StandardisedRates_v2_sensitivity	Invert mvs mva mvv so the attributable risks calculate comparator - mirtazapine
*************************************

*** Log
capture log close sens
local date: display %dCYND date("`c(current_date)'", "DMY")
log using "logs/sensitivity_standardisedrates_`date'.txt", text append name(sens)
*******
...
** REPEAT CODE FOR THE FOLLOWING THREE SENSITIVITY ANALYSES:
*** NO PRIOR RECORDS FOR PALLIATIVE CARE
*** NO PRIOR SELF HARM
*** IGNORING BASELINE BIPOLAR AND SCHIZOPHRENIA

** Loop over different sensitivity analyses
forval Z = 1/3 {

	frames reset

	** START: load dataset (restriction will depend on the analysis)
	use if cohort<=4 & inc_thirdad==1 using data/clean/final_combined.dta // for this analysis drop patients who start two antideps on index

	keep cohort patid keep1 eligible bipolar schizophrenia inc_switch90 depress_12 ageindex sex endreason30 index cod_L1 serioussh_int enddate30 palliative selfharm bl_intselfharm 


	** CREATE required variables:  start stop outcome agecat sex cohort patid
	egen agecat = cut(ageindex), at(18, 35(10)85, 100) icodes label // first group wide as few events

	gen died = endreason30==1
	gen start = index

	gen SHm = serioussh_int <= enddate30 // serious self harm

	egen newstop = rowmin(enddate30 serioussh_int)
	gen time = newstop - index


	** keep/drop patients according to the specific analysis
	/* Definition of keep1: 
	...
	*/
	...
	if `Z' == 1 {
		** no prior palliative:
		keep if keep1==1
		drop if palliative==1
		local tag nopalliative
	}
	...
	if `Z' == 2 {
		** no prior self harm:
		...
		local tag noselfharm
	}
	
	if `Z' == 3 {
		** ignore baseline bipolar and schizophrenia
		keep if eligible==1 & inc_switch90==1 & depress_12==1
		local tag ignorebipolarschiz
	}
	...
	** Keep if positive follow-up
	drop if stop <= start // 2021-03-25 change from <
	...
	** Create pairwise indicator variables
	gen mvs = 0 if cohort==1
	replace mvs = 1 if cohort==2

	gen mva = 0 if cohort==1
	replace mva = 1 if cohort==3

	gen mvv = 0 if cohort==1
	replace mvv = 1 if cohort==4
	...
	** Repeat, calculating rate difference for each cohort by sex
	...
	forval X=1/2 {
		...
		di sc_V`X'
		*di "vs S: " sc_S "; vs A: " sc_A "; vs V: " sc_V
	...
	*** Export as table
	export delim using "outputs/sh30_standardised_selfharm_`tag'.csv", replace


	** Close loop for sensitivity analysis
}
