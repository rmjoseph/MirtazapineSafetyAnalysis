* Created 2021-01-05 by Rebecca Joseph at the University of Nottingham
*************************************
* Name:	BaselineCharacteristics
* Creator:	RMJ
* Date:	20210105
* Desc:	Outputs a baseline characteristics table
* Version History:
*	Date	Reference	Update
*	20210105	BaselineCharacteristics	Run only on first dataset; change variable order
*	20210106	BaselineCharacteristics_v2	Calculate % ignoring missing values
*	20210106	BaselineCharacteristics_v2	Rows for follow-up time and no. events
*	20210121	BaselineCharacteristics_v2	Add anxiolytics (was omitted)
*************************************
...
file open outputfile using "outputs/BaselineCharacteristicsTables_`time'.txt", write replace
*** Open log file
capture log close baselinecharas
log using "logs/results_baselinecharas_`date'.txt", text append name(baselinecharas)
...
file write outputfile "-----------------------------------------" _n
file write outputfile "Table generated using keep1"  _n
...
foreach Z of varlist severe depscale alcoholmisuse anxiety mentalhealthservices eatingdis insomnia intellectualdisab personalitydis selfharm bl_intselfharm substmisuse {
	BINARY `Z', row("`Z', n(%)")
	}	
...
gen time = (enddate6 - index)*12/365.25
	
gen over2 = (enddate6 - index) >= (365.25*2)

CONTINUOUS time, row("Follow-up time (months), median (IQR)") 
BINARY over2, row("Number of patients with 2+ years of follow-up, n(%)")
keep if died==1
BINARY over2, row("Number and proportion of deaths occuring in patients with 2+ years of follow-up, n(%)")
keep if died_c==2
BINARY over2, row("Number and proportion of cancer deaths occuring in patients with 2+ years of follow-up, n(%)")
